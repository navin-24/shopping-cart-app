...
class DeliveryNew extends MY_Controller {
    ...
    function index() {
        ...
        $viewArr['db_mobileNumber'] = $this->session->userdata('logged_in')['db_mobileNumber'];
        $viewArr['deliveryboy_name'] = $this->session->userdata('logged_in')['deliveryboy_name'];
        $this->load->view('header', $viewArr);
        $this->load->view('deliverynew');
    ...
    public function sendOTP() {
        ...
        $delivery_validation = $this->delivery_model->validate();
        if($delivery_validation == FALSE){
            echo json_encode(array('status' => 'failed', 'message' => strip_tags(form_error('db_mobileNumber'))));
            return false;
        } else {
            ...
            $deliveryboy = $this->delivery_model->getDeliveryBoyName($db_mobileNumber);
            $deliveryboyName = $deliveryboy['name'];
            $deliveryboyStatus = $deliveryboy['status'];
            if ($deliveryboyName) {
                if(!$deliveryboyStatus){
                    $msg = 'Your Mobile Number is Disabled. Please contact admin';
                ...
                if($sentMobileResponse == 'success' || 1){
                    ...
                    if($response){
                        if ($resend) {
                            ...
                            $msg = "<h4 class='heading'>Hi {$deliveryboyName} </h4>Please enter the OTP sent to " . $db_mobileNumber;
                        }
    ...
    public function verifyOTP() {
        ...
        if ($otp == $otpDetails['otp_value']) {
            ...
            $sessionData['db_mobileNumber'] = $db_mobileNumber;
            $deliveryboy = $this->delivery_model->getDeliveryBoyName($db_mobileNumber);
            $sessionData['deliveryboy_name'] = $deliveryboy['name'];
    ...
    public function deliveryDetails(){/*print_r($this->input->post());die;*/
        ...
        $deliveryboy = $this->delivery_model->getDeliveryBoyName($db_mobileNumber);
        $delivery_date = $this->delivery_model->getDeliveryDetailsByEmail($customerDetails['email']);
        $deliveryboyName = $deliveryboy['name'];
        if (!$customerDetails) {
        ...
        if(!$delivery_date){
            echo json_encode(array('status' => 'failed', 'message' => "Oops. It seems this Customer has not placed any order yet"));
            return false;
        }
        if(date("Y-m-d", strtotime($delivery_date)) > date('Y-m-d')){
            echo json_encode(array('status' => 'failed', 'message' => "Oops. It seems there is no Order Scheduled for today"));
            return false;
        }
        $delivery_details['customer_name'] = $customerDetails['first_name']." ".$customerDetails['last_name'];
        $delivery_details['delivery_boy_name'] = $deliveryboyName;
